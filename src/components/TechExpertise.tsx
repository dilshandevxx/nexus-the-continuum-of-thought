...
export function TechExpertise({ disableRipple = false }: { disableRipple?: boolean }) {
  return (
    <StarFieldBackground 
        className="py-24" 
    ...
    >
      <div className="relative z-10 mx-auto max-w-7xl py-16">
        <div className="mb-16 px-8 text-center md:px-12">
            ...
            <h3 className="text-3xl font-bold md:text-5xl text-white">
              Tech Expertise
            </h3>
        </div>

      <div className="flex flex-wrap justify-center gap-6 px-4 md:px-20 text-white">
        {TECH_STACK.map((tech, i) => (
            <MagneticTag key={i}>
                <motion.div
                    ...
                    className="cursor-pointer rounded-full border border-white/10 bg-white/5 px-8 py-4 text-lg font-medium backdrop-blur-sm transition-colors hover:bg-white hover:text-black hover:shadow-[0_0_20px_white]"
                >
                    {tech}
                </motion.div>
            </MagneticTag>
